...
openai_llm = ChatOpenAI(
    model="gpt-4o", temperature=0 ,rate_limiter=rate_limiter, api_key=api_key
)
...
def generate_queries(state: OverallState, config: RunnableConfig) -> dict[str, Any]:
    ...
    structured_llm = openai_llm.with_structured_output(Queries)
...
async def research_company(
    ...
    result = await openai_llm.ainvoke(p)
    state_update = {
...
def gather_notes_extract_schema(state: OverallState) -> dict[str, Any]:
    ...
    structured_llm = openai_llm.with_structured_output(state.extraction_schema)
...
def reflection(state: OverallState) -> dict[str, Any]:
    ...
    structured_llm = openai_llm.with_structured_output(ReflectionOutput)
